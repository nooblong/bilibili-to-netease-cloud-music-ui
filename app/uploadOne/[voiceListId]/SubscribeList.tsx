...
import {api, cn, replaceImageUrl} from "@/lib/utils";
      ...
      <CardContent>
        <ScrollArea className="p-2 rounded-lg bg-black/20 border border-cyan-500/20">
          {subscribe && subscribe.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-4 mb-3 sm:mb-0">
                    <div className="relative">
                      <div
                        className="absolute -inset-1 bg-gradient-to-r from-cyan-500 to-purple-500 rounded-full opacity-75 group-hover:opacity-100 blur transition-opacity"></div>
                    ...
                    <div className="font-medium text-sm md:text-base text-cyan-100 font-mono">
                      ...
                      {item.upName}
                      <span
                        className="ml-2 text-xs px-2 py-0.5 rounded-full bg-purple-500/30 border border-purple-500/50">
                                                {item.type}
                                            </span>
                    </div>
                  </div>
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent className="bg-slate-900 border-red-500/50 text-white">
                        <form action={deleteSubscribe}>
                          ...
                          <AlertDialogFooter>
                            <AlertDialogCancel
                              className="bg-slate-800 text-white border-red-500/50 hover:bg-red-900/30">
                              取消
                            </AlertDialogCancel>
                            <AlertDialogAction
                              type="submit"
            ...
            <div
              className="text-cyan-300/50 text-sm font-mono py-6 text-center border border-dashed border-cyan-500/30 rounded-lg">
              <div className="flex flex-col items-center">
                <div
                  className="w-12 h-12 rounded-full border-2 border-cyan-500/30 flex items-center justify-center mb-2">
